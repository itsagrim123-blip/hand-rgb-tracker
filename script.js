...
function makeHandState(label, color) {
  return { label, color, points: [], rawPoints: [], palm: null, previousPalm: null, targetPalm: null, velocity: { x: 0, y: 0, speed: 0 }, trail: [], rotation: 0, shieldScale: 0, confidence: 0, gesture: 'NONE', candidateGesture: 'NONE', candidateFrames: 0, previousGesture: 'NONE', pinching: false, pinchStarted: false, pinchEnded: false, pinchFrames: 0, releaseFrames: 0, pinchDistance: 1, rawPinchDistance: 1, visible: false, intensity: 0, lastSeen: 0, anchorLock: null };
}
...
// `canvasPoint()` already applies the one required horizontal mirror for the CSS-mirrored camera.
// This function only converts that stable visible-screen coordinate into the Three.js world plane.
function mediaPipeToWorld(point, camera) {
  const distance = camera.position.z;
  const height = 2 * Math.tan(THREE.MathUtils.degToRad(camera.fov / 2)) * distance;
  const width = height * camera.aspect;
  return new THREE.Vector3(
    (point.x / innerWidth - .5) * width,
    (.5 - point.y / innerHeight) * height,
    clamp(-(point.z || 0) / Math.max(innerWidth, 1) * 4, -1.2, 1.2)
  );
}
...
function processHands(result, now) {
  ...
  (result.landmarks || []).forEach((landmarks, index) => {
    ...
    if (hand.pinchStarted) beginGrab(hand, points[8]);
    if (hand.pinchEnded) releaseGrab(hand);
    state.detectedHands.push({ order: index + 1, label: hand.label, index: { ...points[8] } });
  });
  ['left', 'right'].forEach(key => { if (!seen.has(key)) { const hand = state[key]; hand.visible = false; hand.pinchStarted = false; hand.pinchEnded = hand.pinching; hand.pinching = false; } });
}
...
class ThreeCubeSystem {
  constructor() {
    ...
    this.camera = new THREE.PerspectiveCamera(52, innerWidth / innerHeight, .1, 30);
    this.camera.position.z = 5.5;
    this.renderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
    this.renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
    this.renderer.setSize(innerWidth, innerHeight);
    threeRoot.append(this.renderer.domElement);
    this.scene.add(new THREE.HemisphereLight(0xb8edff, 0x13071b, 2));
    const key = new THREE.DirectionalLight(0xffffff, 2.2); key.position.set(2, 3, 4); this.scene.add(key);
    this.cube = new THREE.Mesh(new THREE.BoxGeometry(.7, .7, .7), new THREE.MeshStandardMaterial({ color: 0x45ddf7, emissive: 0x07374c, metalness: .45, roughness: .28 }));
    this.cube.position.set(0, .1, 0); this.scene.add(this.cube);
    this.grabbers = []; this.initialDistance = 0; this.initialScale = 1; this.targetScale = 1;
  }
  resize() { this.camera.aspect = innerWidth / innerHeight; this.camera.updateProjectionMatrix(); this.renderer.setSize(innerWidth, innerHeight); }
  handPoint(hand) { return mediaPipeToWorld(handAnchor(hand), this.camera); }
  update(dt) {
    const hands = [state.left, state.right];
    for (const hand of hands) {
      if (!hand.points.length) continue;
      const point = this.handPoint(hand);
      if (hand.pinchStarted && !this.grabbers.includes(hand.label) && point.distanceTo(this.cube.position) < .72) this.grabbers.push(hand.label);
      if (hand.pinchEnded) this.grabbers = this.grabbers.filter(label => label !== hand.label);
    }
    this.grabbers = this.grabbers.filter(label => { const hand = label === 'LEFT' ? state.left : state.right; return hand.pinching && hand.intensity > .1; });
    if (this.grabbers.length === 1) {
      const hand = this.grabbers[0] === 'LEFT' ? state.left : state.right;
      this.cube.position.lerp(this.handPoint(hand), 1 - Math.pow(.003, dt));
      this.initialDistance = 0;
    } else if (this.grabbers.length === 2) {
      const left = this.handPoint(state.left), right = this.handPoint(state.right), distance = left.distanceTo(right);
      if (!this.initialDistance) { this.initialDistance = Math.max(distance, .01); this.initialScale = this.cube.scale.x; }
      this.cube.position.lerp(left.add(right).multiplyScalar(.5), 1 - Math.pow(.003, dt));
      this.targetScale = clamp(this.initialScale * distance / this.initialDistance, .25, 4);
      const next = lerp(this.cube.scale.x, this.targetScale, 1 - Math.pow(.003, dt)); this.cube.scale.setScalar(next);
    } else { this.initialDistance = 0; this.cube.rotation.y += dt * .42; }
...
function render(now) {
  ...
  smoothHands(dt, now); threeCube.update(dt); calculatePanelTransform(dt, now); updateObjects(dt);
...
addEventListener('resize', () => { resize(); threeCube.resize(); });

resize();
seedObjects();
const threeCube = new ThreeCubeSystem();
